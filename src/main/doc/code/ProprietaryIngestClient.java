...
/**
 * Example code adding proprietary information to the metadata of the digital object.
 * @author hartmann-v
 */
public class ProprietaryIngestClient implements IMetadata4Ingest {
  ...
  /** Digital Object ID (DOID) of the data ingested to the repository. The DOID will be
   * determined during the registration process.
   */
  String digitalObjectId;
  /**
   * Example code for ingesting a directory to repository with some proprietary
   * changes during ingest.
   * Before executing this code you should execute the following program:
   * bin/setupRepo init -a
   * At the end please save the settings. When you finished the program successfully
   * your settings will be stored at: 
   * HomeDir/.repoClient/RepoSettings.xml
   * Once the settings stored the ingest should work without further changes.
   * 
   * @param args 
   */
