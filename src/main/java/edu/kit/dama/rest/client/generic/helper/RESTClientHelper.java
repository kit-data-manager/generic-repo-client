...
//		this.dataOrganizationRESTURL = baseURL + REST_DATA_ORGANIZATION_PATH;
  ...
  /**
   * The getSpecificUser method searches and retrieves the user requested by its
   * Long format ID
   *
   ...
   * investigation
   * @return An object of Investigation which contains the information about the
   ...
   */
  /**
   * The method createStudy will create a new study only after checking whether
   * the requested study exists or not. If the Study exists then this existing
   ...
   * Study existing under nanoscopy group will be searched and returned or Study
   * existing under eCodicology group will be searched and returned
   *
   * @param topic The requested topic to check if a study with the similar topic
   * exists or not
   *
  ...
  /**
   * This method can be used to create the base metadata for the digtial object.
   *
   ...
   * @param digitalObject The default digital object which will be added to the
   * investigation
   *
  ...
  /**
   * This method will return the list of the DigitalObjects requested via their
   * digitalObjectIDs
   *
   * @param group The group under which the digital object will be created
  ...
  public IngestInformation createIngestEntity(String digitalObjectID, String accessMethod, String groupId) {
    ...
    try {
      StagingRestClient stagingClient = new StagingRestClient(this.stagingRESTURL, this.context);
      IngestInformationWrapper postNewIngest = stagingClient.createIngest(digitalObjectID, accessMethod, groupId);
  ...
  /**
   * This method is used to actually ingest/upload the digital data into the KIT
   * Data Manger. The method uses ADALAPI for uploading the data.
   *
   * @param filesToIngest List of files that needs to be ingested in the KIT DM
   * @param ingestInfo The IngestInformation object for which the files will be
   * ingested to KIT DM.
   *
   *
  ...
  public CommandStatus performIngestADALAPI(File filesToIngest, IngestInformation ingestInfo) {
    /*
     * This method requires to some refactoring to check the clientResponse when the Status for the ingest is requested
     * 
     */
    // the updateIngestStatus is wrong the methods needs to be fixed in the BaseUserClient - updateIngestInformationById
    AbstractFile uploadedFile;
    IngestInformation ingestInformation;
    CommandStatus commandStatus;

    ClientResponse updateIngestStatus = updateIngestStatus(ingestInfo.getId(), INGEST_STATUS.PRE_INGEST_RUNNING.getId());
//    if (updateIngestStatus.getStatusInfo().getFamily().compareTo(Response.Status.Family.SUCCESSFUL) != 0) {
//      LOGGER.error("Error while setting ingest status to '{}'!", INGEST_STATUS.PRE_INGEST_RUNNING);
//      commandStatus = new CommandStatus(new Exception("Error during initialization of ingest!"));
...
//    }
    ...
    try {
      ...
      Collection<AbstractFile> list = directoryToUpload.list();
      for (AbstractFile directoryOrFile : list) {
        if (directoryOrFile.isDirectory()) {
          uploadedFile = directoryOrFile.uploadDirectory(remoteLocation);
        } else {
          uploadedFile = directoryOrFile.uploadFileToDirectory(remoteLocation);
        }
        if (uploadedFile == null) {
          updateIngestStatus(ingestInfo.getId(), INGEST_STATUS.PRE_INGEST_FAILED.getId());
          commandStatus = new CommandStatus(Status.FAILED, null, ingestInfo);
  ...
  /**
   * Method to ingest data using the DataTransferClient. The number of parallel
   * transfer tasks are defined in datamanger.xml file, they can be changed as
   * per your requirement. This file should be placed in the resources folder of
   * your client project
   *
...
//      DataOrganizationUtils.printTree(factoryIngestContainer.getFileTree().getRootNode(), true);
   ...
   *
   * @return ClientResponse specifying whether the status update was successful
   * or not
   */
   ...
   * KIT Data Manager
   *
   * @return The the total count of ingests entities available from the database
   */
  ...
  /**
   * This methods will request for the complete details of the IngestInformation
   * using the IDs received from the getIngestInformationIDs() methods
   *
  ...
  /**
   * This method is used to create the download entity in the KIT Data Manager,
   * so that the requested digital data can be downloaded
   *
   * @param digitalObjectID The digitalObject for which the download entity will
   * be created
   *
   * @param accessMethod The access method with which to download the data,
   * i.e.for e.g. Webdav
   * @param groupId The groupId the download belongs to.
   * @return DownloadInformation for the digital data that needs to be
   * downloaded
   */
  public DownloadInformation createDownloadEntity(String digitalObjectID, String accessMethod, String groupId) {
    DownloadInformation downloadInformation = null;
    try {
      StagingRestClient stagingClient = new StagingRestClient(this.stagingRESTURL, this.context);
      DownloadInformationWrapper createdDownload = stagingClient.createDownload(digitalObjectID, accessMethod, groupId);
  ...
  /**
   * This method is used to get the IngestInformation identified by the digital
   * object ID from the KIT Data Manager
   *
   ...
   * @param dataToDownload The downloadinformation specifying the actual digital
   * data that you wish to download
   ...
   * @return the abstract file object which was downloaded to the local machine
   */
   ...
   * C:\mydownloadeddata\
   *
   * @return AbstractFile containing the location of data where it is downloaded
   */
